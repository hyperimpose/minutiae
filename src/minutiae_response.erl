%%--------------------------------------------------------------------
%% Copyright (C) 2023-2026 hyperimpose.org
%%
...
-type http() :: http_duckduckgo_search()
              | http_html()
              | http_fallback()
              | http_file_audio()
              | http_file_image()
...
-type http_torolainla() ::
        #{m         => http_torolainla,
          t         => binary(),  % Title generated by minutia
          direct    => binary(),  % Direct url to the file
          explicit  => float()}.  % Adult content
...
http(#{<<"@">>           := <<"http:toro.lain.la">>,
       <<"t">>           := T,
       <<"direct">>      := Direct,
       <<"explicit">>    := Explicit}) ->
    {ok, #{m           => http_torolainla,
           t           => T,
           direct      => Direct,
           explicit    => binary_to_float(Explicit)}};
